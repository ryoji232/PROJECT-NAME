...
function initBarcodeScanner() {
    ...
    function process(raw) {
        ...
        //      When on the Books page, open the per-book showModal directly.
        //   2. Fall back to AJAX /copies/scan/{code} so scanning works on ANY
        //      page in the app (dashboard, borrowings list, etc.).
        //      After resolving the copy, branch on copy.status:
        //        • available → open the global borrowModal (in every page layout)
        //        • borrowed  → look up the active borrowing and open the return modal
        if (/^[A-Z0-9]{4,12}$/.test(barcode)) {
            // ── Fast path: client-side index (Books page only) ───────────────
            ...
            // ── Slow path: AJAX lookup (works on every page) ─────────────────
            fetch(`/copies/scan/${encodeURIComponent(barcode)}`, { credentials: 'same-origin' })
                ...
                .then(function (data) {
                    if (!data || !data.success || !data.copy) {
                        toastr.error('Barcode not recognised. Please check the barcode and try again.');
                        return;
                    }

                    const copy = data.copy;

                    // ── Case 1: Copy is available → open the borrow modal ────
                    if (copy.status === 'available') {
                        // The borrowModal is included in every page via
                        // partials/modals.blade.php, so this always works.
                        showBorrowModal(copy);
                        return;
                    }

                    // ── Case 2: Copy is borrowed → look up active borrowing ──
                    if (copy.status === 'borrowed') {
                        fetch(`/copies/${copy.id}/active-borrowing`, { credentials: 'same-origin' })
                            .then(function (r) {
                                if (!r.ok) throw new Error('HTTP ' + r.status);
                                return r.json();
                            })
                            .then(function (result) {
                                if (result && result.success && result.borrowing_id) {
                                    showReturnModal(result.borrowing_id);
                                } else {
                                    toastr.warning('This copy appears to be borrowed but no active record was found. Please check manually.');
                                }
                            })
                            .catch(function (err) {
                                toastr.error('Could not load return details: ' + err.message);
                            });
                        return;
                    }

                    // ── Case 3: Any other status (damaged, lost, etc.) ───────
                    toastr.warning('This copy is currently not available (status: ' + copy.status + ').');
                })
                .catch(function (err) {
                    toastr.error('Scanner error: ' + err.message);
                });
            return;
        }
