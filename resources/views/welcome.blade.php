        ...
        <div class="row">
            <div class="col-md-4">
                <div class="stats-box text-center">
                    <div class="stats-badge">{{ $books->count() ?? 0 }}</div>
                    <div class="fw-semibold">TOTAL BOOKS</div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stats-box text-center">
                    <div class="stats-badge">{{ $borrowings->count() ?? 0 }}</div>
                    <div class="fw-semibold">CURRENTLY BORROWED</div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="stats-box text-center">
                    <div class="stats-badge">{{ $withPenaltiesOrOverdue->count() }}</div>
                            ...
                            <li class="list-group-item">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <em>"{{ $borrowing->book->title ?? 'Unknown Book' }}"</em>
                                    </div>
                                    <small class="text-muted">
                                        {{ \Carbon\Carbon::parse($borrowing->borrowed_at ?? $borrowing->created_at)->format('M d, Y h:i A') }}
                                    </small>
                                </div>
                            </li>
                        @endforeach
...
<div class="row">
    <div class="col-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body p-0">
                <ul class="list-group list-group-flush">
                    @forelse ($withPenaltiesOrOverdue as $borrowing)
                        @php
                            ...
                            <div class="d-flex justify-content-between align-items-start">
                                <div class="flex-grow-1">
                                    <div class="d-flex align-items-center mb-2">
                                        <strong class="me-2">{{ $borrowing->student_name }}</strong>
                                        <span class="badge-days">{{ $borrowing->days_overdue }} days overdue</span>
                                    </div>
                                    <div class="mb-2">
                                        Book: <em>"{{ $borrowing->book->title ?? 'Unknown Book' }}"</em>
                                    </div>
                                    <div class="d-flex align-items-center">
                                        <span class="badge-penalty me-2">Penalty: ₱{{ number_format($penalty, 2) }}</span>
                                        <small class="text-muted">
                                            ({{ $periodRange }} period)
                                        </small>
                                    </div>
                                    <small class="text-muted">
                                        Due: {{ Carbon::parse($borrowing->due_date)->format('M d, Y') }}
                                    </small>
                                </div>
                                <div class="text-end">
                                    <div class="text-danger fw-bold mb-1">₱{{ number_format($penalty, 2) }}</div>
                                    <small class="text-muted">{{ $borrowing->days_overdue }} days</small>
                                </div>
                            </div>
                        </li>
                    @empty
...
<!-- Most Popular Book -->
@if($mostBorrowed && $mostBorrowed->book)
<div class="row">
    <div class="col-12">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <h5 class="text-purple">{{ $mostBorrowed->book->title }}</h5>
                        <p class="text-muted mb-2">by {{ $mostBorrowed->book->author }}</p>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="border rounded p-2">
                            <div class="h5 text-purple mb-0">{{ $mostBorrowed->borrow_count }}</div>
                            <small class="text-muted">Times Borrowed</small>
                        </div>
                    </div>
                    <div class="col-md-3 text-center">
                        <div class="border rounded p-2">
                            <div class="h5 text-primary mb-0">{{ $mostBorrowed->book->copies }}</div>
