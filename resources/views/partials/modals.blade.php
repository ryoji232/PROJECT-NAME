{{-- =====================================================
     ...
     Global modals: Return confirmation + Borrow (scanner)

     Two scanner flows are supported here:
       1. COPY-BARCODE flow  (#borrowModal)       — alphanumeric copy barcode
          → POST /borrow/by-barcode
       2. BOOK-ID flow       (#globalBorrowModal) — numeric book-id barcode
          → POST /borrow  (book_id)
     Both borrow modals are wired by the global scanner engine in app.blade.php.

     Return flow (#barcodeReturnModal):
       • openReturnModal(borrowingId)  — called from borrowings table (borrowings.index)
       • window.__openReturnModalForBook(bookId) — called by the global scanner
         Fetches the active borrowing from /books/{id}/current-borrowing then
         populates and opens #barcodeReturnModal, which submits via AJAX to
         POST /return/{borrowingId}.
     ===================================================== --}}

{{-- ── 1. Return Confirmation Modal ────────────────────────────────────── --}}
...
{{-- ── 2. Copy-Barcode Borrow Modal (existing scanner flow, books page) ─── --}}
...
{{-- ── 3. Global Book-ID Borrow Modal (scanner on dashboard / any page) ── --}}
{{--
     Opened by window.__openBorrowModalForBook(bookId).
     Uses POST /borrow (borrow.store) with book_id — the controller picks
     any available BookCopy automatically, exactly like the books page form.
--}}
