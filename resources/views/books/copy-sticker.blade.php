...
<head>
    <meta charset="utf-8">
    <title>Copy Sticker - {{ $copy->copy_number ?? $copy->id }}</title>
    <style>
...
<body>
    <div class="sticker">
        <div style="font-weight:700; font-size:14px; margin-bottom:4px;">{{ $copy->book->title ?? 'Book' }}</div>
        <div style="font-size:12px; color:#333; margin-bottom:6px;">{{ $copy->copy_number ?? 'Copy' }}</div>
        <div class="barcode">
            <?php $printable = \App\Models\BookCopy::normalizeBarcode($copy->barcode); ?>
            {!! DNS1D::getBarcodeHTML($printable, 'C128', 3, 90) !!}
        </div>
        <div class="code">{{ $printable }}</div>
    </div>
