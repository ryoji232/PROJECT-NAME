...
<style>
    ...
    .alpha-jump-bar a:hover,
    .alpha-jump-bar a.active-letter {
        background: #198754;
        color: #fff;
        border-color: #198754;
    ...
    .alpha-jump-bar a.disabled-letter {
        color: #adb5bd;
        background: #f8f9fa;
        border-color: #dee2e6;
        pointer-events: none;
    ...
    @media (max-width: 767px) {
        ...
        .shelf-letter-badge { width: 38px; height: 38px; font-size: 1.1rem; }
        .alpha-jump-bar a { width: 28px; height: 28px; font-size: 0.75rem; }
        .alpha-jump-bar { top: 56px; gap: 3px; padding: 0.5rem 0.75rem; }
    }
</style>
...
<form action="{{ route('books.store') }}" method="POST" class="add-book-form">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-2">
            <input type="number" name="copies" class="form-control" placeholder="Copies (max 10)"
...
<div class="d-flex justify-content-between align-items-center mb-2 flex-wrap gap-2">
    <h2 id="books" class="mb-0">Available Books</h2>
    <div class="search-box mb-0" style="min-width:260px;">
        <input type="text" id="bookSearch" class="form-control" placeholder="🔍 Search by title…">
    </div>
</div>

@php
    // Sort alphabetically by title, then group by first letter
    $sortedBooks    = $books->sortBy(fn($b) => strtolower($b->title));
    $grouped        = $sortedBooks->groupBy(fn($b) => strtoupper(substr(trim($b->title), 0, 1)))->sortKeys();
    $presentLetters = $grouped->keys()->all();
@endphp

<!-- Sticky alphabet jump bar -->
<div class="alpha-jump-bar" id="alphaJumpBar">
    @foreach(range('A','Z') as $letter)
        @if(in_array($letter, $presentLetters))
            <a href="#shelf-{{ $letter }}" class="alpha-letter" data-letter="{{ $letter }}">{{ $letter }}</a>
        @else
            <a class="disabled-letter" aria-disabled="true">{{ $letter }}</a>
        @endif
    @endforeach
</div>
...
<div id="availableBooks">
    @forelse($grouped as $letter => $letterBooks)
        <div class="shelf-section" id="shelf-{{ $letter }}" data-shelf="{{ $letter }}">

            <!-- Section header -->
            <div class="shelf-label">
                <div class="shelf-letter-badge">{{ $letter }}</div>
                <hr class="shelf-rule">
                <span class="shelf-label-text">
                    {{ $letterBooks->count() }} {{ Str::plural('title', $letterBooks->count()) }}
                </span>
            </div>

            <!-- Book cards for this letter -->
            <div class="row g-3 shelf-cards">
                @foreach($letterBooks as $book)
                    @php $notReturnedCount = $book->borrowings->whereNull('returned_at')->count(); @endphp
            ...
            <hr style="border-top: 2px solid #c3e6cb; margin: 8px 0 2.25rem;">
...
{{-- ════════════════════════════════════════════════════
     ALL MODALS  (kept exactly as original)
     ════════════════════════════════════════════════════ --}}
@foreach($books as $book)
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <div>
                        <h5 class="modal-title" id="showModalLabel{{ $book->id }}">{{ $book->title }}</h5>
                ...
                <div class="modal-body">
                        ...
                        <form class="borrow-form mb-3" data-book-id="{{ $book->id }}">
                            ...
                            <div class="d-flex flex-column gap-2">
                                ...
                                <button type="submit" class="btn btn-success btn-sm w-100 borrow-btn"
                                        data-book-id="{{ $book->id }}">BORROW</button>
                            </div>
                        </form>
                    @endif
                    ...
                    <div class="d-flex flex-column gap-2">
                        <button class="btn btn-warning btn-sm w-100"
                                data-bs-toggle="modal" data-bs-target="#historyModal{{ $book->id }}"
                                data-book-id="{{ $book->id }}">
                            IN USE (<span id="modal-in-use-count-{{ $book->id }}">{{ $notReturnedCount }}</span>) — View History
                        </button>
                        <button type="button" class="btn-info-custom btn-sm w-100"
                                data-bs-toggle="modal" data-bs-target="#copiesModal{{ $book->id }}">
                            VIEW COPIES &amp; BARCODES
                        </button>
                        <a href="{{ route('books.edit', $book->id) }}" class="btn btn-primary btn-sm w-100">EDIT BOOK</a>
    ...
    <div class="modal fade" id="historyModal{{ $book->id }}" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">📋 Full Borrow History — {{ $book->title }}</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
    ...
    <div class="modal fade" id="copiesModal{{ $book->id }}" tabindex="-1"
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Copies — {{ $book->title }}</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
...
document.addEventListener("DOMContentLoaded", function () {
        ...
        if (bookId && modal.id.startsWith('historyModal')) {
            ...
            .then(data => {
                ...
                records.forEach(record => {
                    ...
                    if (record.returned_at) { badge = `<span class="history-status-returned">✓ Returned</span>`; returnedLine = `<div class="mt-1" style="color:#0a3622;font-size:.82rem;">↩ Returned: ${fmt(record.returned_at)}</div>`; }
                    else if (record.is_overdue) { badge = `<span class="history-status-overdue">⚠ Overdue</span>`; }
                    else { badge = `<span class="history-status-active">📖 Active</span>`; }
    ...
    document.addEventListener('submit', function (e) {
        ...
        .then(r => r.json())
        .then(data => {
            if (data.success) { updateBookCard(bookId, data.book.available_copies, data.active_borrowings); showNotification('success', data.message); form.reset(); }
    ...
    function updateBookCard(bookId, availableCopies, activeBorrowings) {
        ...
        if (badge) { badge.textContent = availableCopies > 0 ? `Available: ${availableCopies}` : 'Fully Borrowed'; badge.className = `badge ${availableCopies > 0 ? 'bg-success' : 'bg-secondary'} available-badge`; }
    ...
    /* ── Search ─────────────────────────────────────────
       Hides individual cards AND entire shelf sections
       when no books match the query.
    ── */
        ...
        searchInput.addEventListener("input", function () {
            ...
            document.querySelectorAll('.shelf-section').forEach(section => {
                ...
                section.querySelectorAll('.book-card').forEach(card => {
                    ...
                    // Highlight matched text (strip marks first, then re-apply)
                    if (titleEl) {
                        const plain = titleEl.textContent;
                        if (q && matches) {
                            const esc = q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                            titleEl.innerHTML = plain.replace(new RegExp(`(${esc})`, 'gi'), '<mark>$1</mark>');
                        } else {
                ...
                // Hide/show entire shelf (label + plank)
                section.style.display = anyVisible ? '' : 'none';
            });

            // Dim jump-bar letters whose section is now hidden
            document.querySelectorAll('#alphaJumpBar .alpha-letter').forEach(link => {
                const sec = document.getElementById(`shelf-${link.dataset.letter}`);
                const hidden = sec && sec.style.display === 'none';
                link.classList.toggle('disabled-letter', hidden);
                if (hidden) link.classList.remove('active-letter');
            });
        });
    }
    ...
    const observer = new IntersectionObserver(entries => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const letter = entry.target.dataset.shelf;
                document.querySelectorAll('#alphaJumpBar .alpha-letter').forEach(l => {
                    l.classList.toggle('active-letter', l.dataset.letter === letter);
                });
            }
        });
    }, { rootMargin: '-10% 0px -80% 0px' });
    document.querySelectorAll('.shelf-section').forEach(s => observer.observe(s));
    ...
    document.addEventListener('click', function (e) {
        ...
        fetch(form.action, { method:'POST', body:new FormData(form), headers:{ 'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':'{{ csrf_token() }}' } })
        .then(r => r.json())
        .then(data => {
