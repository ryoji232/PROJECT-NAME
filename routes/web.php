...
Route::middleware(['auth:librarian'])->group(function () {
    ...
    // Active borrowing lookup — used by the barcode scanner to resolve
    // a "borrowed" copy's barcode directly into a return modal.
    Route::get('/copies/{copyId}/active-borrowing', function ($copyId) {
        ...
        if (! $borrowing) {
            return response()->json([
                'success'     => false,
                'message'     => 'No active borrowing found for this copy.',
                'borrowing_id' => null,
            ]);
        }
